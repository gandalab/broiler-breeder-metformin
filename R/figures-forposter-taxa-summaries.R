...
### plot all genera and color only top 10
...
# change axis to italic
ggpar(p, font.legend = c(fontface = "italic"))

## save with normal legend
ggsave(filename = "R/figures/forposter-genus-treatment-relabun.png", plot = p, dpi = 600)
...
p <- ps %>% 
  ...
  comp_heatmap(
    colors = heat_palette(palette = "BluYl"), 
    taxa_side = "top",
    name = "CLR",
...
### Sophia's code to use viridis colors
library(ComplexHeatmap)
# get matrix
mat <- ps %>% 
  tax_transform("clr", rank = "Phylum") %>% 
  otu_get() %>% as.data.frame() %>% as.matrix()

# get prevalence
library(vegan)
prev <- decostand(mat, "pa")
prev <- colSums(prev) 
prev <- prev / 113

pdf("R/figures/forposter-heatmap.pdf", width = 13, height = 10)
# make heatmap
Heatmap(mat,
        col = viridis(20),
        ...
        column_names_rot = 45,
        heatmap_legend_param = list(title = "CLR", fontsize = 20),
        top_annotation = HeatmapAnnotation(Prev = anno_barplot(prev, which = "column")),
